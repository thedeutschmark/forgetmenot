/**
 * LLM adapter — Gemini-only.
 *
 * History: v0.1.35 added OpenAI + Anthropic adapters for provider A/B
 * evaluation. v0.1.36 reverted to Gemini-only per the complexity
 * guardrail after the A/B showed Gemini flash-lite ties or beats the
 * alternatives on the deterministic voice-discipline rubric. Carrying
 * three provider branches was maintenance cost without a quality win.
 * The Gemini OpenAI-compatible endpoint keeps the request/response
 * shape simple — one fetch, one JSON, one path.
 */
...
export async function chatCompletion(
  ...
  if (!res || !res.ok) {
    ...
    throw new Error(`LLM gemini returned ${res?.status ?? "?"}: ${lastErrBody.slice(0, 200)}`);
  }
  ...
  // Normalize finish_reason — Gemini's OpenAI-compat layer returns
  // "stop"/"length"/"content_filter"/"tool_calls". Unknown values flatten
  // to null so callers don't have to case on edge strings.
