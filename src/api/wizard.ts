...
async function handleFinish(res: ServerResponse, rawBody: string) {
  ...
  try {
    const settingsRes = await authFetch(`${_config.authUrl}/bot-runtime/settings`, {
      method: "PUT",
  ...
  try {
    const policyRes = await authFetch(`${_config.authUrl}/bot-runtime/policy`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        installationId: _config.installationId,
...
<div class="card">
  ...
  <p class="sub">These keep the bot well-behaved out of the gate. Change anytime from the toolset.</p>
...
async function finish() {
  ...
  try {
    ...
    if (d.ok) {
      st.className = 'status status-ok';
      st.innerHTML = 'Bot is running! <a href="${esc(config.authUrl.replace("auth.", "toolset."))}/tools/chat-bot" target="_blank" style="color:#4ade80">Open toolset dashboard</a>';
