...
// ── Step 0: Sync src/version.ts from package.json ────────────────────
// Ensures the version string the runtime reports matches what's in the
// npm manifest. Before this sync existed, bumping a release required
// editing 3 files; we drifted once on v0.1.40 prep. Now only
// package.json is the human-touched source of truth.
...
const currentVersionTs = fs.existsSync(VERSION_TS_PATH)
  ? fs.readFileSync(VERSION_TS_PATH, "utf8")
  : "";
if (currentVersionTs !== versionTsBody) {
  fs.writeFileSync(VERSION_TS_PATH, versionTsBody);
  console.log(`[build] Synced src/version.ts → ${pkg.version}`);
}
